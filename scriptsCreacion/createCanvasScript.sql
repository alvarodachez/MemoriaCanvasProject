DROP TABLE TAREA CASCADE CONSTRAINTS;
DROP TABLE OBJETIVO CASCADE CONSTRAINTS;
DROP TABLE PROYECTO CASCADE CONSTRAINTS;
DROP TABLE USUARIOPROYECTO CASCADE CONSTRAINTS;
DROP TABLE USUARIO CASCADE CONSTRAINTS;
--CREACION TABLAS
CREATE TABLE TAREA (
    idTarea NUMBER(10) PRIMARY KEY,
    duracion NUMBER(10),
    ESTADO VARCHAR2(30),
    idUsuario NUMBER(10),
    nombre VARCHAR2(60)
);

CREATE TABLE USUARIO (

    idUsuario NUMBER(10) PRIMARY KEY,
    apodo VARCHAR(20),
    contraseña VARCHAR(6)

);

CREATE TABLE PROYECTO (
    idProyecto NUMBER(10) primary key,
    nombre VARCHAR2(60)
);

CREATE TABLE USUARIOPROYECTO(
    idUsuario NUMBER(10),
    idProyecto NUMBER(10)
);
CREATE TABLE OBJETIVO (
    idObjetivo NUMBER(10) primary key,
    nombre VARCHAR2(60),
    prioridad VARCHAR2(20),
    idProyecto NUMBER(10)
);

--APLICAR RESTRICCIONES
ALTER TABLE TAREA ADD CONSTRAINT fk_usuario FOREIGN KEY (idUsuario) REFERENCES USUARIO(idUsuario) ON DELETE CASCADE ;
ALTER TABLE TAREA ADD CONSTRAINT check_estados CHECK(ESTADO in ('PENDIENTE','REALIZANDO','FINALIZADA'));
ALTER TABLE TAREA ADD CONSTRAINT unique_nombre unique(nombre);
ALTER TABLE USUARIO ADD CONSTRAINT unique_apodo unique(apodo);
ALTER TABLE USUARIOPROYECTO ADD CONSTRAINT uspr_pk PRIMARY KEY (idUsuario,idProyecto);
ALTER TABLE USUARIOPROYECTO ADD CONSTRAINT us_fk FOREIGN KEY (idUsuario) REFERENCES USUARIO(idUsuario) ON DELETE CASCADE;
ALTER TABLE USUARIOPROYECTO ADD CONSTRAINT pr_fk FOREIGN KEY (idProyecto) REFERENCES PROYECTO(idProyecto) ON DELETE CASCADE;

ALTER TABLE OBJETIVO ADD CONSTRAINT pr1_fk FOREIGN KEY (idProyecto) REFERENCES PROYECTO(idProyecto) ON DELETE CASCADE;
ALTER TABLE OBJETIVO ADD CONSTRAINT check_prioridad CHECK(PRIORIDAD in ('ALTA','NORMAL','BAJA'));

--INSERTAR VALORES POR DEFECTO (necesario para el funcionamiento del programa)
INSERT INTO USUARIO (idUsuario, apodo, contraseña)
VALUES(1,'admin','admin');
INSERT INTO TAREA (idTarea, duracion, ESTADO, idUsuario, nombre)
VALUES(1,1,'FINALIZADA',1,'InitialTask');
INSERT INTO PROYECTO (idProyecto, nombre)
VALUES(1,'firstProject');
INSERT INTO USUARIOPROYECTO (idUsuario, idProyecto)
VALUES(1,1);
INSERT INTO OBJETIVO(idObjetivo, nombre, prioridad, idProyecto)
VALUES (1,'firstObjective','BAJA',1);

commit;
